              ...
              ><a-col :md="8" :sm="24">
                <a-form-item label="字段值类型">
                  <a-select
                    :allowClear="true"
                    style="width: 100%"
                    v-model="queryParam.valueType"
                    placeholder="请选择字段值类型"
                  >
                    <a-select-option v-for="(item, index) in valueTypeData" :key="index" :value="item.code">{{
                      item.name
                    }}</a-select-option>
                  </a-select>
                </a-form-item>
    ...
    <a-card :bordered="false">
      ...
      >
        <template class="table-operator" slot="operator" v-if="hasPerm('SsuIssueExtendAttribute:add')">
          <a-button
            type="primary"
            v-if="hasPerm('SsuIssueExtendAttribute:add')"
            icon="plus"
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical" v-if="hasPerm('SsuIssueExtendAttribute:edit') & ($store.getters.admintype == 1)" />
          <a-popconfirm
            v-if="$store.getters.admintype == 1"
            placement="topRight"
            title="确认删除？"
            @confirm="() => SsuIssueExtendAttributeDelete(record)"
          >
            <a>删除</a>
          </a-popconfirm>
        </span>
