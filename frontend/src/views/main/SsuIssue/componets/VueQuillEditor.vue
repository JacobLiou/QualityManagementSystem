<template>
  <div class="vue-quill-editor">
    ...
    <a-upload :customRequest="customRequest" class="quill-upload" name="image">
      <div>
        <a-icon type="plus" />
...
export default {
  ...
  methods: {
    ...
    handleImgUpload() {
      document.querySelector('.quill-upload .anticon-plus').click()
    },
    customRequest(data) {
      console.log(data)
      const { quill } = this.$refs.mwQuillEditor
      const render = new FileReader()
      render.readAsDataURL(data.file)
      render.onload = function (e) {
        const imageBase64 = e.target.result
        const pos = quill.getSelection().index
        // 获取光标所在位置
        // 插入图片，res为服务器返回的图片链接地址
        quill.insertEmbed(pos, 'image', imageBase64)
        // 调整光标到最后
