  ...
  <a-card class="add">
    ...
    <section class="form_1">
      <a-form-model ref="ruleForm" :labelCol="labelCol" :wrapperCol="wrapperCol" :model="form" :rules="rules">
        ...
        <a-form-model-item label="预计解决日期" prop="forecastSolveTime">
          <a-date-picker
            style="width: 100%"
            placeholder="请选择预计解决日期"
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        id: null,
        title: '', // 问题简述，
        forecastSolveTime: undefined, // 预计解决日期
      ...
      rules: {
        ...
        forecastSolveTime: [{ required: true, message: '请选择预计解决日期', trigger: 'change' }],
  ...
  methods: {
    ...
    initForm(value) {
      ...
      this.form.forecastSolveTime = moment().format('YYYY-MM-DD') // 预计解决日期
