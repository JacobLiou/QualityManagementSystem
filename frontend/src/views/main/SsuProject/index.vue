          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="项目名称">
                <a-input v-model="queryParam.projectName" allow-clear placeholder="请输入项目名称"/>
              </a-form-item>
    ...
    <a-card :bordered="false">
      <s-table
        ...
        <template class="table-operator" slot="operator" v-if="hasPerm('SsuProject:add')" >
          <a-button type="primary" v-if="hasPerm('SsuProject:add')" icon="plus" @click="$refs.addForm.add()">新增项目</a-button>
        </template>
        <span slot="action" slot-scope="text, record">
          <a v-if="hasPerm('SsuProject:edit')" @click="$refs.editForm.edit(record)">编辑</a>
          <a-divider type="vertical" v-if="hasPerm('SsuProject:edit') & hasPerm('SsuProject:delete')"/>
          <a-popconfirm v-if="hasPerm('SsuProject:delete')" placement="topRight" title="确认删除？" @confirm="() => SsuProjectDelete(record)">
            <a>删除</a>
          </a-popconfirm>
        </span>
      </s-table>
