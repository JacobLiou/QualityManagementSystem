          ...
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="人员组名称">
                <a-input v-model="queryParam.groupName" allow-clear placeholder="请输入人员组名称"/>
              </a-form-item>
    ...
    <a-card :bordered="false">
      <s-table
        ...
        <template class="table-operator" slot="operator" v-if="hasPerm('SsuGroup:add')" >
          <a-button type="primary" v-if="hasPerm('SsuGroup:add')" icon="plus" @click="$refs.addForm.add()">新增人员组</a-button>
        </template>
        <span slot="action" slot-scope="text, record">
          <a v-if="hasPerm('SsuGroup:edit')" @click="$refs.editForm.edit(record)">编辑</a>
          <a-divider type="vertical" v-if="hasPerm('SsuGroup:edit') & hasPerm('SsuGroup:delete')"/>
          <a-popconfirm v-if="hasPerm('SsuGroup:delete')" placement="topRight" title="确认删除？" @confirm="() => SsuGroupDelete(record)">
            <a>删除</a>
          </a-popconfirm>
        </span>
      </s-table>
